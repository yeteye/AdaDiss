...
Fig 2  : GNN 隐层嵌入 UMAP（数据域 + 细胞类型着色，两子图）
Fig 3  : 训练动态曲线（Loss 分量 + Val/Train F1）
Fig 4  : 方法定量对比（Acc / F1-macro / F1-weighted 柱状图）
Fig 5  : 每个方法的归一化混淆矩阵
Fig 6  : 空间预测图（多方法散点图）
Fig 7  : 细胞类型比例对比（scRNA vs 各模型，堆叠条形图）
Fig 8  : 逐类别 F1 热力图
Fig 9  : 预测置信度分布（小提琴 + CDF）
Fig 10 : Moran's I 空间自相关柱状图

修复清单
--------
generate_all_thesis_figures：Fig 2 embedding 切分逻辑错误
  原代码：emb[:n_scrna] / emb[n_scrna:]  ← 错误
    gnn_results["embeddings"] 已经是纯 xenium embedding，再切会越界
  修复：使用 gnn_results["scrna_embeddings"] 和 ["embeddings"] 两个独立字段
  （models.py predict_xenium 已同时返回这两个字段）
"""
...
def compute_metrics(
    ...
    """计算 Accuracy, F1 (macro/weighted/per-class), Cohen's Kappa, AUC（可选）。"""
...
def compare_all_methods(
    ...
    """汇总所有方法的定量指标（基于 scRNA 验证集，不用 Xenium 标签）。"""
    rows = []
...
# ══════════════════════════════════════════════════════════
# 2. Fig 2 — GNN 隐层嵌入 UMAP
# ══════════════════════════════════════════════════════════

def plot_embedding_umap(
    scrna_embeddings: np.ndarray,    # (n_scrna, h_dim) scRNA 隐层
    xenium_embeddings: np.ndarray,   # (n_xenium, h_dim) Xenium 隐层
    ...
    """Fig 2：GNN 隐层嵌入 UMAP（域对齐 + 细胞类型，两子图）。"""
    from umap import UMAP
...
def plot_training_curves(
    ...
    """Fig 3：各模型 Loss 分量 + Val/Train F1 随 epoch 变化曲线。"""
    ...
    for col, (name, history) in enumerate(all_histories.items()):
        ...
        # 修复：使用 .get(..., 0) 防止旧缓存 history 中字段缺失
        train_f1 = [h.get("train_f1_macro", 0) for h in history]
...
def plot_method_comparison(
    ...
    """Fig 4：所有方法的 Accuracy / F1-macro / F1-weighted / Kappa 对比。"""
...
def plot_confusion_matrix(
    ...
    fig, ax = plt.subplots(
        figsize=(max(10, len(cell_types) * 0.7), max(8, len(cell_types) * 0.65))
    )
    sns.heatmap(
...
def plot_spatial_predictions(
    ...
    fig.legend(handles=handles, loc="lower center",
               ncol=min(len(cell_types), 4), fontsize=8, frameon=False,
...
def plot_cell_type_proportions(
    ...
    """Fig 7：scRNA vs 各模型 Xenium 预测的细胞类型比例（堆叠条形图）。"""
    n_types = len(cell_types)
...
def generate_all_thesis_figures(
    ...
    修复：Fig 2 embedding 切分逻辑
    --------------------------------
    原代码（错误）：
        all_gnn_emb[name] = res["embeddings"]   # 已经是 xenium-only embedding
        plot_embedding_umap(emb[:n_scrna], emb[n_scrna:], ...)  # 切分无意义

    修复后：
        直接使用 gnn_results[name]["scrna_embeddings"] 和 ["embeddings"] 两个字段
        （models.py 的 predict_xenium 已同时返回这两个字段）
    """
    out = Path(output_dir)
    ...
    all_pred_indices["TopACT"] = topact_results.get("pred_indices",
                                  topact_results.get("indices", np.array([])))
    all_confidences["TopACT"]  = topact_results.get("confidence", np.array([]))

    # ── 汇总 metrics ─────────────────────────────────────
    metrics_dict = {}
    for method, val_pred in val_pred_per_method.items():
        metrics_dict[method] = compute_metrics(
            val_labels, val_pred, n_classes=len(cell_types)
        )
    ...
    try:
        ...
        print("  ⚠ umap-learn not installed, skip Fig 1")

    # ── Fig 2 ────────────────────────────────────────────
    # 修复：使用 scrna_embeddings 和 embeddings 两个独立字段
    print("\n[Fig 2] GNN embedding UMAP...")
    for name, res in gnn_results.items():
        scrna_emb = res.get("scrna_embeddings")
        xen_emb   = res.get("embeddings")
        if scrna_emb is None or xen_emb is None:
            print(f"  ⚠ {name}: embedding 字段缺失，跳过 Fig 2")
            continue
        try:
            plot_embedding_umap(
                scrna_emb, xen_emb,
            ...
            print("  ⚠ umap-learn not installed, skip Fig 2")
            break
    ...
    if xenium_coords is not None and len(xenium_coords) > 0:
