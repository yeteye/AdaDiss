# ============================================================
# 【必须最先运行】把worker函数写入磁盘文件
# spawn模式下子进程只能从.py文件import函数，不能从Notebook的__main__获取
# ============================================================
...
class GAT(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, heads=8, dropout=0.3, num_layers=2, use_skip=True):
        super().__init__()
    ...
    def forward(self, data):
        x, edge_index = data.x, data.edge_index
        identity = self.skip_proj(x) if (self.use_skip and self.num_layers >= 2) else None
        for i, conv in enumerate(self.convs[:-1]):
            x = conv(x, edge_index)
            x = self.bns[i](x)
            x = F.elu(x)
...
def train_model(model, data, device, optimizer, epochs=500, patience=50, verbose=False):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            val_acc  = (pred_val[data.val_mask] == data.y[data.val_mask]).float().mean().item()
...
def evaluate_model(model, data, device):
    ...
    precision, recall, _, _ = precision_recall_fscore_support(test_true, test_pred, average="macro")
    return {
...
def save_model_and_results(model, config, model_name, training_history=None):
    ...
    if "heads"    in config: model_filename += f"_heads{config[\'heads\']}"
    if "use_skip" in config: model_filename += f"_skip{config[\'use_skip\']}"
    model_filename += ".pt"
    ...
    if "heads"    in config: result_row["heads"]    = config["heads"]
    if "use_skip" in config: result_row["use_skip"] = config["use_skip"]
    ...
    for attempt in range(10):
        try:
            if os.path.exists(excel_path):
                ...
                results_df   = pd.concat([existing_df, pd.DataFrame([result_row])], ignore_index=True)
            else:
...
def save_detailed_results(model_name, config, training_history, test_true, test_pred, class_names_list):
    ...
    if "heads" in config: param_str += f"_heads{config[\'heads\']}"
    save_path = os.path.join(MODEL_DIRS[model_name], f"{model_name}_{param_str}.json")
    detailed  = {
        "model_name":   model_name,
        "config":       {k: v for k, v in config.items() if k not in ["test_true","test_pred"]},
...
def build_model(model_name, params, in_dim, out_dim):
    if model_name == "MLP":
        return MLPWithDropout(in_dim, params["hidden_dim"], out_dim,
                              params["dropout"], params["num_layers"])
    elif model_name == "GCN":
        return GCN(in_dim, params["hidden_dim"], out_dim,
                   params["dropout"], params["num_layers"])
    elif model_name == "GraphSAGE":
        return GraphSAGEWithNorm(in_dim, params["hidden_dim"], out_dim,
                                 params["dropout"], params["num_layers"])
    elif model_name == "GAT":
        return GAT(in_dim, params["hidden_dim"], out_dim,
                   params["heads"], params["dropout"],
                   params["num_layers"], params["use_skip"])
    elif model_name == "GATv2":
        return GATv2Model(in_dim, params["hidden_dim"], out_dim,
                          params["heads"], params["dropout"], params["num_layers"])
    raise ValueError(f"未知模型: {model_name}")


# ── Worker函数（spawn子进程的入口）────────────────────────────

def worker_process(gpu_id, task_queue, result_queue, data_dict, model_name, class_names_list):
    """
    每个GPU对应一个独立进程。
    ...
    while True:
        ...
        try:
            ...
            trained_model, train_losses, val_losses, train_accs, val_accs, best_val_acc = train_model(
                model, data, device, optimizer, epochs=500, patience=50, verbose=False
            )

            eval_results     = evaluate_model(trained_model, data, device)
            config           = {**params, "best_val_acc": best_val_acc, "num_params": num_params, **eval_results}
            training_history = {
            ...
            save_detailed_results(model_name, config, training_history,
                                  eval_results["test_true"], eval_results["test_pred"], class_names_list)

            result_queue.put((idx, config, None))
            print(f"[GPU {gpu_id}] 完成任务 #{idx} | Val={best_val_acc:.4f} Test={eval_results[\'test_acc\']:.4f}", flush=True)
...
# 写入磁盘
with open("gnn_worker.py", "w", encoding="utf-8") as f:
    f.write(worker_code)

print("✓ gnn_worker.py 已写入磁盘")
print("  （只需运行此Cell一次，之后重启kernel也不需要重新运行，文件已持久化）")